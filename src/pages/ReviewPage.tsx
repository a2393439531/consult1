...
export function ReviewPage() {
  ...
  useEffect(() => { Promise.all(Array.from({ length: 11 }, (_, index) => loadChapter(String(index + 1)))).then((chapters) => setQuestions(chapters.flatMap((chapter) => chapter.questions))).catch(() => setQuestions([])) }, [])
